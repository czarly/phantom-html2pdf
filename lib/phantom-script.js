...
page.open(page.libraryPath + "/skeleton.html", function (status) {
  ...
  page.evaluate(function(jsPath) {
    ...
    var paperBorder = args.paperBorder || '1cm';
    
    var margin = {
	top: args.paperBorderTop || paperBorder,
	bottom: args.paperBorderBottom || args.paperBorder,
	left: args.paperBorderLeft || args.paperBorder,
	right: args.paperBorderRight || args.paperBorder
    };
    
  /* Alter pagesize according to specified header/footer data */
    var defaultFormat = {format: args.paperFormat, orientation: args.paperOrientation, margin: margin};
			 
  if(args.paperWidth !== 'false'){
    defaultFormat = {width: args.paperWidth, height: args.paperHeight, margin: margin};
  }
